...
if($arParams['AVAILABLE_SORT'] && $arParams['SORT'] && $arParams['SORT_ORDER']){
	...
	$sort = 'SHOWS';
	if((array_key_exists('sort', $_REQUEST) && array_key_exists(ToUpper($_REQUEST['sort']), (array)$arAvailableSort)) || (array_key_exists('sort', $_SESSION) && array_key_exists(ToUpper($_SESSION['sort']), $arAvailableSort)) || $arParams['ELEMENT_SORT_FIELD']){
		if($_REQUEST['sort']){
