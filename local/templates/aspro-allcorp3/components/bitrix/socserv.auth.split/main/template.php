						...
						<tr class="soc-serv-personal">
							...
							<td class="split-item-actions">
								<?if (in_array($arUser["ID"], $arResult["ALLOW_DELETE_ID"])):?>
									<a class="btn transparent remove-soc btn-sm" href="?action=delete&user_id=<?=$arUser["ID"]."&".bitrix_sessid_get()?>" onclick="return confirm('<?=GetMessage("SS_PROFILE_DELETE_CONFIRM")?>')" title=<?=GetMessage("SS_DELETE")?>>
										<?=TSolution::showIconSvg('close', SITE_TEMPLATE_PATH.'/images/svg/Close_sm.svg');?>
										<?=GetMessage("SS_DELETE")?>
									</a>
								<?endif;?>
							</td>
						</tr>
